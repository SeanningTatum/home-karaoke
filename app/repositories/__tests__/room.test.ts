...
describe("RoomRepository.updateGuestReorder", () => {
  it.effect("fails with RoomNotFoundError when missing", () => {
    const stub = { select: () => chainable([]) };
    return Effect.gen(function* () {
      const repo = yield* RoomRepository;
      const exit = yield* Effect.exit(
        repo.updateGuestReorder({ roomId: "missing", allowGuestReorder: true })
      );
      expect(Exit.isFailure(exit)).toBe(true);
      if (Exit.isFailure(exit)) {
        const failure = Cause.failureOption(exit.cause);
  ...
  it.effect("updates the flag and returns the updated row", () => {
    let selectCall = 0;
    const responses = [
      [{ id: "r1", status: "open", allowGuestReorder: false }],
      [{ id: "r1", status: "open", allowGuestReorder: true }],
    ];
    const stub = {
      select: () => chainable(responses[selectCall++]),
      update: () => chainable(undefined),
    ...
    return Effect.gen(function* () {
      const repo = yield* RoomRepository;
      const result = yield* repo.updateGuestReorder({
        roomId: "r1",
        allowGuestReorder: true,
      });
      expect(result).toEqual(responses[1][0]);
