...
// group-karaoke Phase 1 demo data — hosted by the admin fixture above, with
// a couple of user-fixture guests already in the queue history.
const ROOM_FIXTURE: RoomFixture = {
  id: "seed-room-1",
  code: "KQ7-3FP",
  hostId: "seed-admin",
  status: "open",
...
const SONG_FIXTURES: SongFixture[] = [
  {
    videoId: "dQw4w9WgXcQ",
...
const ROOM_SONG_FIXTURES: RoomSongFixture[] = [
  {
    id: "seed-room-song-1",
    videoId: "dQw4w9WgXcQ",
    singerNickname: "Alice",
    addedByUserId: "seed-user",
    played: true,
  },
  {
    id: "seed-room-song-2",
...
async function buildSql(): Promise<string> {
  ...
  lines.push(
    `INSERT OR IGNORE INTO room (id, code, host_user_id, status, allow_guest_reorder, created_at) VALUES (` +
      [
        sqlString(ROOM_FIXTURE.id),
        sqlString(ROOM_FIXTURE.code),
        sqlString(ROOM_FIXTURE.hostId),
        sqlString(ROOM_FIXTURE.status),
        ROOM_FIXTURE.allowGuestReorder ? 1 : 0,
        now,
      ].join(", ") +
      `);`,
  );

  for (const s of SONG_FIXTURES) {
    lines.push(
      `INSERT OR IGNORE INTO song (video_id, title, channel, thumbnail_url, embeddable, duration_seconds, created_at) VALUES (` +
        [
          sqlString(s.videoId),
          sqlString(s.title),
          sqlString(s.channel),
          sqlString(s.thumbnailUrl),
          s.embeddable ? 1 : 0,
  ...
  for (const rs of ROOM_SONG_FIXTURES) {
    lines.push(
      `INSERT OR IGNORE INTO room_song (id, room_id, video_id, singer_nickname, added_by_user_id, played_at, created_at) VALUES (` +
        [
          sqlString(rs.id),
          sqlString(ROOM_FIXTURE.id),
          sqlString(rs.videoId),
          sqlString(rs.singerNickname),
          rs.addedByUserId ? sqlString(rs.addedByUserId) : "NULL",
          rs.played ? now : "NULL",
          now,
        ].join(", ") +
        `);`,
...
function describeMarkdown(): string {
  ...
  lines.push(
    "Seeded data: the accounts above (Better Auth `user` + credential " +
      "`account` rows), plus a demo karaoke room (`room` `" +
      ROOM_FIXTURE.code +
      "`, hosted by `" +
      ROOM_FIXTURE.hostId +
      "`, status `" +
