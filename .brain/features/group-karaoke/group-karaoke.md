...
| `VideoNotEmbeddableError` | `youtube.resolveVideo` route — `status.embeddable === false` (paste-a-link safety net) | BAD_REQUEST |
| `VideoNotFoundError` | youtube service — empty `videos.list`/oEmbed 404 | NOT_FOUND |

> **Non-embeddable videos are kept out, two layers deep.** Videos whose owner disabled embedding (`status.embeddable === false`, e.g. Sing King) genuinely can't play in the IFrame — YouTube blocks them with error 150/101 (empirically confirmed, see [`verifications/2026-07-21-singking-embeddable-fix.md`](verifications/2026-07-21-singking-embeddable-fix.md)). So (1) `youtube.search` runs a cheap `videos.list?part=status` pass over its `search.list` hits (1 quota unit for the batch) and **filters non-embeddable results out**, and `SongRepository.getCachedPicks` filters `embeddable = true` too; (2) `youtube.resolveVideo` rejects `embeddable === false` with `VideoNotEmbeddableError` as a safety net for the paste-a-link path (the keyless oEmbed fallback can't report embeddability, so an oEmbed-resolved paste is assumed playable and the player's `onError` is the last line of defense).
...
|------|------|-------------|
| 2026-07-21 | fix | Sing King videos queued but never played — their owner disabled embedding, so YouTube blocks them in the IFrame (error 150/101), confirmed empirically. `youtube.search` now runs a `videos.list?part=status` pass and **filters non-embeddable results out** (degrades to unfiltered on lookup failure); `SongRepository.getCachedPicks` filters `embeddable = true`; `youtube.resolveVideo` rejects `embeddable === false` with `VideoNotEmbeddableError` as a paste-a-link safety net. (Supersedes the initial same-day attempt that removed the check on a wrong "flag is unreliable" premise.) Extracted `resolveVideoProgram` for testability; `app/trpc/routes/__tests__/youtube.test.ts` + `app/services/__tests__/youtube.test.ts` filter tests + `e2e/group-karaoke-queue.spec.ts`. Live-preview verification (real Data API): [`verifications/2026-07-21-singking-embeddable-fix.md`](verifications/2026-07-21-singking-embeddable-fix.md). 521 tests. |
