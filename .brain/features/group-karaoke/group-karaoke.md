...
|------|------|-------------|
| 2026-07-21 | fix | Sing King (and other karaoke channels) were un-queueable — `youtube.resolveVideo` hard-rejected `status.embeddable === false` with `VideoNotEmbeddableError`, but the Data API flag is unreliable and those videos embed fine. Removed the server-side check (+ the now-dead `VideoNotEmbeddableError`, its `tagToTRPC` mapping, `VIDEO_NOT_EMBEDDABLE_MESSAGE_PREFIX`, client branch, and `not_embeddable` i18n key). Player `onError` remains the runtime ground truth (toast + auto-skip). Extracted `resolveVideoProgram` for testability; added `app/trpc/routes/__tests__/youtube.test.ts` (5 deterministic regression tests) + `e2e/group-karaoke-queue.spec.ts` (add-to-queue golden path). Live-preview browser verification with the real Data API: [`verifications/2026-07-21-singking-embeddable-fix.md`](verifications/2026-07-21-singking-embeddable-fix.md). 518 tests. |
