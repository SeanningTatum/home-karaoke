...
| `app/lib/song-title.ts` | `cleanSongTitle`, `formatDuration`, `normalizeForMatch` — pure **display-only** title cleaning for the TV/queue surfaces (feat-014). `cleanSongTitle(title, channel)` → `{ song, artist, raw }`: strips karaoke/quality noise (`\| Karaoke Version`, `(HD Karaoke)`, `(Official Video)`, `(4K Remaster)`, `[HD]`, `M/V`, channel tails), then splits on the first *spaced* hyphen/en-dash/em-dash — split direction comes from the data (left segment matching the normalized channel ⇒ left is the artist, else the karaoke `Song - Artist` convention). Noise detection uses unicode `\p{L}\p{N}` classes so non-Latin titles survive; falls back to the raw title if cleaning empties it. **Never persist the cleaned string** — raw stays the stored/queue/history value |
