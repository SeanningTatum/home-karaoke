...
| `room` | `app/trpc/routes/room.ts` | `create` (protected), `get` (public — guests resolve a room by code before authenticating), `listMine` (protected query, feat-013 — the caller's rooms newest-first with per-room played-song counts via `RoomRepository.listRoomsByHost`; rejects anonymous sessions with `FORBIDDEN` "Sign in to see your sessions" like `create`; strips `hostUserId` from the rows before returning — feeds the dashboard sessions rail), `close` (protected, host-only), `setGuestReorder` (protected, host-only — persists the `allowGuestReorder` toggle to D1), `recordPlayed` (protected, host-only — writes `room_song` history via `SongRepository.recordRoomSong` + `markPlayed`) |
