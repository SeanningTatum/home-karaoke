...
- **Video not found** — `youtube.resolveVideo` fails `VideoNotFoundError` (404) with an inline error on the search/paste form. (Embeddability is NOT enforced server-side — the Data API `embeddable` flag is unreliable, so non-embeddable videos are queued and the player's `onError` auto-skips them at playback; see [`../features/group-karaoke/group-karaoke.md`](../features/group-karaoke/group-karaoke.md).)
