...
- **`search`** — `search.list`, query biased toward "karaoke" (`withKaraokeBias` in `app/lib/youtube.ts`). A D1 7-day cache (`SongRepository.getCachedPicks`, keyed on `search_log.normalizedQuery`) is checked before spending API quota on a repeat query.
...
- **Keyless oEmbed fallback**: `getVideo` (and therefore `resolveVideo`) falls back to the public YouTube oEmbed endpoint — no key required — when the Data API key is absent or a lookup needs only title/author/thumbnail. oEmbed doesn't return `embeddable`/`duration`. Note: embeddability is never enforced server-side (the Data API flag is unreliable — Sing King etc. report `false` yet play fine), so every resolved video is treated as playable-until-proven-otherwise and the IFrame player's `onError` auto-skips genuinely non-embeddable ones at playback.
