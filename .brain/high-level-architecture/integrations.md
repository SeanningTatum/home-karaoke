...
- **Keyless oEmbed fallback**: `getVideo` (and therefore `resolveVideo`) falls back to the public YouTube oEmbed endpoint — no key required — when the Data API key is absent or a lookup needs only title/author/thumbnail. oEmbed doesn't return `embeddable`/`duration`, so a resolved-via-oEmbed video is treated as playable-until-proven-otherwise (no `VideoNotEmbeddableError` short-circuit).
