...
## Group-karaoke authorization (feat-007)

### Anonymous guest sessions

Better Auth's **`anonymous`** plugin (`app/auth/server.ts`) is layered on top of the existing email/password + `admin` setup. `signIn.anonymous()` (client: `app/auth/client.ts`'s `anonymousClient()`) provisions a real `session` + `user` row — just `user.isAnonymous = true` — so `protectedProcedure` treats an anonymous guest exactly like a real account: `ctx.auth.user` is non-null either way, and nothing in the middleware distinguishes them (see `app/trpc/routes/youtube.ts`'s `search` procedure comment for the canonical explanation).

**This is a real gap, not a designed restriction**: nothing currently stops an anonymous session from calling `room.create` directly (only the dashboard UI's "create room" affordance implies a real account is expected). Listed here rather than in "Known gaps" below because it's feature-specific; close it by adding an explicit `ctx.auth.user.isAnonymous` check in `room.create` if guest-hosted rooms need to be prevented.

### Host / guest roles (per-room, not global RBAC)

Room-level roles are **not** part of the `user.role` RBAC system (`"user" | "admin"`) — they're computed per-request from room ownership:
...
- **Host** = the account that called `room.create` (`room.hostUserId`). A host can also open `/join/:code` on a second device (e.g. phone) and gets the same `"host"` role there — the join route's guest UI shows an extra host-only "Controls" tab in that case (`app/routes/join/$code.tsx`'s `isHost` check), but that's a UX affordance only.
- **Guest** = everyone else. Gets a nickname (own input, or the account's real name if not anonymous) and joins the room's roster.
- **Enforcement is at the Durable Object**, not the UI: `canPerform` (`app/lib/room-state.ts`) is the single source of truth for whether a given role/message pair is allowed (host-only playback controls; guest reorder gated additionally by the room's `allowGuestReorder` toggle). A guest's client can still *render* a disabled control, but the DO rejects a disallowed message regardless of what the client sent.

### WS identity headers

The WebSocket upgrade route (`app/routes/api/room.$code.ws.ts`) resolves identity via `getSession` + `RoomRepository`, then forwards it to the `KARAOKE_ROOM` Durable Object stub as plain headers on the cloned request — the DO never calls Better Auth or a repository itself:

| Header | Set from | Used for |
|--------|----------|----------|
| `x-user-id` | `session.user.id` | Roster identity, `addedByUserId` on queued songs |
| `x-nickname` | Guest's chosen nickname, or `session.user.name` | Display name in roster/queue |
| `x-role` | `session.user.id === room.hostUserId ? "host" : "guest"` | `canPerform` role gate |
| `x-room-id` | `room.id` (D1) | Lets the DO's idle alarm know which D1 row to close |
| `x-allow-guest-reorder` | `room.allowGuestReorder` (D1) | Seeds the DO's live setting on a fresh (never-persisted) DO instance |

### Host-only tRPC procedures

`room.close`, `room.setGuestReorder`, and `room.recordPlayed` are `protectedProcedure`s that additionally check `existing.hostUserId === ctx.auth.user.id` and fail with a **direct `TRPCError({ code: "FORBIDDEN" })`** — per `rules/errors.md`'s stated exception, this is procedure-specific authorization control flow, not a domain tagged error, so it bypasses `tagToTRPC` the same way `protectedProcedure`/`adminProcedure` middleware does.
